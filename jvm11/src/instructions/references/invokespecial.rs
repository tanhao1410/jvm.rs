...
impl Instruction for INVOKE_SPECIAL {
    ...
    fn execute(&mut self, thread: Arc<RwLock<Thread>>) {
        ...
        let thread_guard = thread2.write().unwrap();

        let frame = thread_guard.current_frame();

        let frame_mut = frame.borrow_mut();

        let cp = frame_mut.method.constant_pool();
        ...
        // match frame_mut.operand_stack().get_ref_from_top(method.arg_slot_count) {
        ...
        invoke_method(thread_guard, method, thread, frame_mut);
    }
}
