...
pub struct Class {
    ...
    //对于每一个Class ,在堆中都有一个java.lang.Class对象，与之对应
    pub j_class:Option<Arc<RwLock<Object>>>,

    // 源文件名
    pub source_file:Arc<String>,
}
...
impl Class {
    ...
    pub fn array_class(&self) -> Arc<RwLock<Class>> {
        ClassLoader::load_class(self.loader.clone(), self.array_class_name().as_str())
    }
    ...
    pub fn get_field<T:AsRef<str>>(&self, name: T ,desc: T) -> Arc<RwLock<Field>> {

        for field in &self.fields {
            let guard = field.read().unwrap();
            if guard.name.as_str() == name.as_ref()
                && guard.descriptor.as_str() == desc.as_ref() {
                return field.clone();
            }
        }

        if self.super_class_name.as_str() != ""{
            //还需要考虑它的父类与接口
            let parent = self.super_class.as_ref().unwrap();
            let parent = parent.read().unwrap();
            return parent.get_field(name,desc);
        }

        panic!("no field:{}", name.as_ref())
    }
    ...
    pub fn get_instance_method(&self,name:&str,desc:&str)->Option<Arc<Method>>{
        for method in &self.methods {
            if !method.is_static() && method.name.as_str() == name && method.descriptor.as_str() == desc {
                return Some(method.clone());
            }
        }
        None
    ...
    pub fn init_class(&mut self, thread: Arc<RwLock<Thread>>, mut guard: RwLockWriteGuard<Thread>) {
        //先把自己的初始化状态置位true
        ...
        if let Some(clinit) = clinit {
            //创建一个帧,推入栈顶
            let new_frame = Frame::new( thread.clone(), clinit);
            guard.push_frame(new_frame);
        }
        //如果本类是类，执行父类的
