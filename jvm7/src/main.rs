...
fn main() {
    ...
    if cmd.version_flag {
        ...
        cmd.cp_option = "C:/Users/tanhao/CLionProjects/jvm/jvm7/target/debug".to_string();
        //cmd.class = "jvmgo.book.ch07.FibonacciTest".to_string();
        //cmd.class = "jvmgo.book.ch07.LReturn".to_string();
        cmd.class = "jvmgo.book.ch07.ClassInitTest".to_string();
...
fn start_jvm(cmd: CMD) {
    ...
    if let Some(main_method) = main_method{
        //测试解释器
        interpreter::interpret(main_method,cmd.inst_log_flag);
    }else{
