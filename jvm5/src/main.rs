...
fn main() {
    ...
    if cmd.version_flag{
        ...
        cmd.cp_option = "C:/Users/tanhao/CLionProjects/jvm/jvm5/target/debug".to_string();
        cmd.class = "jvmgo.book.ch05.GaussTest".to_string();
...
fn start_jvm(cmd:CMD){
    ...
    //let mut frame = Frame::new(100, 100,thread);
    //test_local_vars(frame.local_vars());
    //test_operand_stack(frame.operand_stack());

    for member_info in class_file.methods().iter(){
        if member_info.name().as_str() == "main" && member_info.descriptor().as_str() == "([Ljava/lang/String;)V"{
            //测试解释器
            interpreter::interpret(member_info);
        }
    }
