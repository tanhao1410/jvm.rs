...
fn main() {
    ...
    if cmd.version_flag {
        ...
        cmd.cp_option = "C:/Users/tanhao/CLionProjects/jvm/jvm9/target/debug".to_string();
        //cmd.class = "jvmgo.book.ch08.PrintArgs".to_string();
        cmd.class = "jvmgo.book.ch09.BoxTest".to_string();
...
fn create_args_array(loader:Arc<RwLock<ClassLoader>>,args:&Vec<String>)->Arc<RwLock<Object>>{
    ...
    //需要把该数组中的元素替换成java中的字符串
    {
        ...
        for i in 0..args.len(){
            let java_string = get_java_str_obj_by_pool(Arc::new(args[i].to_string()), loader.clone());
