...
impl Instruction for INVOKE_INTERFACE {
    ...
    fn execute(&mut self, thread: Arc<RwLock<Thread>>) {
        ...
        let method_ref = cp_guard.get_constant_mut(self.index as usize).get_interface_method_ref_mut();
        ...
        match slot {
            Slot::Ref(obj) => {
                ...
                let method_in_invoke_obj = obj_class.read().unwrap()
                    .lookup_method(method.name.as_str(), method.descriptor.as_str());
