...
impl Instruction for INVOKE_NATIVE {
    fn execute(&mut self, thread: Arc<RwLock<Thread>>) {
        ...
        if let Some(func) = find_native_method(class_name.as_str(),
        ...
        } else if "initialize" == method_name.as_str() && "sun/misc/VM" == class_name.as_str() {
            //为了实现自动拆箱装箱的逻辑
            let vm_class = ClassLoader::load_class(ClassLoader::get_system_class_loader(),
                                                   Arc::new("sun/misc/VM".to_string()));
            let vm_class_guard = vm_class.read().unwrap();

            let saved_props = vm_class_guard.get_field(Arc::new("savedProps".to_string()),
                                                       Arc::new("Ljava/util/Properties;".to_string()));
            ...
            let key = get_java_string(ClassLoader::get_system_class_loader(),
                                      Arc::new("foo".to_string()));
            let val = get_java_string(ClassLoader::get_system_class_loader(),
                                      Arc::new("bar".to_string()));
            ...
            let props_class = ClassLoader::load_class(ClassLoader::get_system_class_loader(),
                                                      Arc::new("java/util/Properties".to_string()));
