...
impl ClassLoader {
    ...
    fn load_java_primitive_classes(loader: &Arc<RwLock<ClassLoader>>) {
        ...
        let mut class_cloader_guard = loader.read().unwrap();
        let map = & class_cloader_guard.class_map;

        for (k,v) in map{
            let class = v.read().unwrap();
            let j_class = class.j_class.as_ref().unwrap();
            let j_class_guard = j_class.read().unwrap();
            let extra = j_class_guard.extra.as_ref().unwrap();
            let name = extra.read().unwrap().name.clone();
        }
    ...
    fn load_java_lang_class(loader: &Arc<RwLock<ClassLoader>>) {
        ...
        let jl_class_class = Self::load_class(loader.clone(), Arc::new("java/lang/Class".to_string()));
            ...
            match field.read().unwrap().descriptor.clone().as_str() {
                ...
                "Ljava/lang/String;" => {
                    match constant_pool.read().unwrap().get_constant(cp_index) {
                        Constant::String(val) => {
                            //怎么拿到常量池呢
                            let java_str = get_java_str_obj_by_pool(
                                Arc::new(val.clone()),
