...
fn get_primitive_class(local_vars: &LocalVars) -> Option<Slots> {
    //该方法时个静态方法，
    ...
    let class_name = get_string_from_slot(loader.clone(), slot);
...
fn get_name0(local_vars: &LocalVars) -> Option<Slots> {
    ...
    match slot {
        Slot::Ref(this) => {
            ...
            if let Some(class) = &read_guard.extra {
                ...
                let j_str_obj = get_java_str_obj_by_pool(class_name, loader.clone());
...
fn desired_assertion_status0(local_vars: &LocalVars) -> Option<Slots> {
