...
impl Instruction for NEW_ARRAY {
    ...
    fn execute(&mut self, thread: Arc<RwLock<Thread>>) {
        ...
        let arr_class = match self.index {
            AT_BOOLEAN =>
                ClassLoader::load_class(class_loader, Arc::new("[Z".to_string())),
            AT_CHAR =>
                ClassLoader::load_class(class_loader, Arc::new("[C".to_string())),
            AT_FLOAT =>
                ClassLoader::load_class(class_loader, Arc::new("[F".to_string())),
            AT_DOUBLE =>
                ClassLoader::load_class(class_loader, Arc::new("[D".to_string())),
            AT_BYTE =>
                ClassLoader::load_class(class_loader, Arc::new("[B".to_string())),
            AT_SHORT =>
                ClassLoader::load_class(class_loader, Arc::new("[S".to_string())),
            AT_INT =>
                ClassLoader::load_class(class_loader, Arc::new("[I".to_string())),
            AT_LONG =>
                ClassLoader::load_class(class_loader, Arc::new("[J".to_string())),
            _ => unreachable!()
        };
