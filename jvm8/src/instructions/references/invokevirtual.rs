                ...
                if method_ref.name.as_str() == "println"{
                    ...
                    match method_ref.descriptor.as_str() {
                        ...
                        "(Ljava/lang/String;)V"=>{
                            ...
                            let j_str_guard = j_str.read().unwrap();

                            let j_str_class = j_str_guard.class.clone();
                            let java_str_class_guard = j_str_class.read().unwrap();
                            let field = java_str_class_guard.get_field(Arc::new("value".to_string()),
                                                                       Arc::new("[C".to_string()));

                            let slot1 =& j_str_guard.fields.slots[field.read().unwrap().slot_id];
                            if let Slot::Ref(chars)= slot1{
                                let guard1 = chars.read().unwrap();
                                let chars = guard1.chars();
